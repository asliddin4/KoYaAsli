...
📊 Advanced statistika"""
            ...
            referral_count = user_stats[5] if user_stats else 0  # referral_count is at index 5
            remaining_referrals = max(0, 10 - referral_count)
...
async def handle_referral_stats(callback: CallbackQuery):
    ...
    try:
        ...
        from database import get_user_stats
        user_stats = await get_user_stats(user_id)
        referral_count = user_stats[5] if user_stats else 0  # referral_count at index 5
        remaining_referrals = max(0, 10 - referral_count)
